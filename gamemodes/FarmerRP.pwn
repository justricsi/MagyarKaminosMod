...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    new msg[128];
    format(msg, sizeof(msg), "%s kilépett a játékból! (%s)", jInfo[playerid][Nev], szDisconnectReason[reason]);
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	ShowPlayerDialog(playerid,d_traktorBerles,DIALOG_STYLE_MSGBOX,"{289AD0}Jármû bérlése","Szeretnéd kibérelni?\n\n{FF8040}40.000Ft/óra","Igen","Nem");
	return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
	{
		...
		case d_FoldBerles: Dialog_FoldBerles(playerid,response);    //ez csak a bérlés menûpontja a fõ dialognak
    }
    return 0;
}
...
public JatekosBeregelt(playerid)
{
    ...
    ShowPlayerDialog(playerid,d_varos,DIALOG_STYLE_LIST,"Kezdõ városok","PalominoCreek\nMontgomery\nBlueBerry\nFortCarson\nLasPayasadas\nBaySide\nAngelPine","Választ","Mégsem");
    return 1;
}
...
Dialog_Fold(playerid,response,listitem)     //ez a fõ dialog ebben van 3 opció
{
if(!response) return 1;
switch(listitem)
{
	case 0:                                 //1. információ itt is ittvan a forciklus
	{
	    ...
	    for(new i=0; i<FoldCount; i++)
	    {
	        if(i == melyikPickUp[playerid])
	        {
				...
				format(foldMeret,sizeof(foldMeret),"|Mérete:____________%d. termõföld___________________|",fInfo[i][id]);
				SendClientMessage(playerid,feher,foldMeret);
				format(foldMeret,sizeof(foldMeret),"|Mérete:____________%d hektár___________________|",fInfo[i][meret]);
				SendClientMessage(playerid,feher,foldMeret);
				format(foldMeret,sizeof(foldMeret),"|Mérete:____________Ára: %d Ft___________________|",fInfo[i][ar]);
				SendClientMessage(playerid,feher,foldMeret);
	...
	case 1:                                 //2. Bérlés
	{
	if(jInfo[playerid][Penz] < 900000) return SendClientMessage(playerid,piros,"((Nincs elég pénzed!))");
		ShowPlayerDialog(playerid,d_FoldBerles,DIALOG_STYLE_LIST,"Bérlés","900.000 Ft/nap","Rendben","Mégsem");
	}
...
{
	RemovePlayerFromVehicle(playerid);
	SendClientMessage(playerid,piros,"((Lejárt a jármû bérlése!))");
}
	...
	for(new i=0; i < FoldCount; i++)
	{
		if(IsPlayerInRangeOfPoint(playerid,1.0,fInfo[i][X],fInfo[i][Y],fInfo[i][Z]))
		{
		    if(picupbanAll[playerid] == 0)
			{
				...
				format(string, sizeof(string), "%d. Termõföld", fInfo[i][id]);
...
CMD:kocsiszin(playerid,params[])
{
	...
	if(mID<400 || mID>611) return SendClientMessage(playerid,piros,"400-611-ig vannak csak kocsik!");
	if(col1<0 || col1>168 && col2<=0 || col2>168) return SendClientMessage(playerid,piros,"0-tól 168-ig van szín!");
...
CMD:nem(playerid,params[])
{
	...
	{
		SendClientMessage(playerid,feher,"Nemed: {00FFFF}Nõ");
	}
	return 1;
}
...
CMD:fegyver(playerid,params[]){
    ...
    if(sscanf(params,"i",weaponID)) return SendClientMessage(playerid,feher,"Használat /fegyver [id]");
